...
- [x] **セッション 2** (hook + marker): `public-leak-guard.sh` 実装 +
  marker file 設置 (LorentzArena + claude-config) + settings.json 登録
  + 動作確認 — **2026-04-09 実施**。11 ケース test matrix
  (clean/email/allowlist/abs_path/ipv4 public/ipv4 private/token_prefix/
  marker-less/non-git/Write content/MultiEdit) を手動実行して全 pass
  確認。Write/Edit/MultiEdit の content 抽出差を jq で吸収。IPv4 は
  `0.0.0.0`, `127.*`, `255.255.255.255`, `10.*`, `172.16-31.*`,
  `192.168.*`, `169.254.*` を allowlist。email は `noreply@anthropic.com`
  / `noreply@github.com` / `support@github.com` を allowlist。token は
  hit しても本体を晒さず先頭 10 文字 + `...` で redact 表示。
  hit 時は `permissionDecision=ask` (deny ではなく ask) で False
  Positive 時の手動承認経路を確保
- [x] **セッション 3** (pre-commit + installer): `public-precommit-runner.sh`
  + `install-public-precommit.sh` + LorentzArena/claude-config で
  install — **2026-04-09 実施**。10 ケース test matrix (clean/
  email real/email allowlist/abs_path/ipv4 public/ipv4 private/token
  prefix/after-reject-retry/--no-verify bypass/tier-b ephemeral
  hit/tier-b absent no-op) を /tmp の test repo 上で全 pass 確認。
  LorentzArena でも fake leak 試行 → reject 確認 → cleanup。
  installer に relative hooks path bug があり修正 (`cd "$REPO" && git rev-parse`
  のあとで相対なら REPO を prefix)
- [x] **セッション 4** (情報分離): Dropbox 正本 → `sensitive-terms.txt`
  symlink + `odakin-prefs/.gitignore` 追加 + `work-network.md` の
  組織名 literal を placeholder へ置換 + pre-commit hook の literal
  catch 動作確認 — **2026-04-09 実施**。`~/Dropbox/claude/sensitive-terms.txt`
  作成 (8 entries)、`~/Claude/odakin-prefs/sensitive-terms.txt` symlink
  配置、`odakin-prefs/.gitignore` 新規作成で除外、work-network.md を
  `<workplace>` / `<workplace_it>` 等 placeholder 化、LorentzArena で
  組織名 literal を含む fake commit → `tier-b/literal` hit で reject
  確認。残存 sensitive literal を含む odakin-prefs tracked file 5 件
  (DESIGN.md, paper-style.md, repos.md, shared-project-keys.md,
  user-profile.md) は `next-steps.md §段階 2` で issue 化済み、
  本件 scope 外
- [ ] **セッション 5 (← ここから次回)** (audit + 全 repo 展開 + setup.sh): `audit-public-repos.sh`
