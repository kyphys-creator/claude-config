...
- 任意ファイル `EXPLORING.md` を新設（ARCHITECTURE.md と同じ「任意ファイル」扱い）
- `EXPLORING.md` は未決定の思考・代替案・option space の棚卸しを置く
- 境界判別ルール: 迷ったら DESIGN.md、完全 option space 広げてる段階だけ EXPLORING.md
- lifecycle: 探索が決定に結晶 → DESIGN.md に promote、EXPLORING.md から削除
- 根拠の詳細は `docs/convention-design-principles.md` §6

### Why

LorentzArena 2+1/DESIGN.md が 500 行超に肥大化し、「残存する設計臭 defer」の決定記録とスマホ UI の思考メモを同時に書く必要が生じた場面で、DESIGN.md に **3 種類** が混在していることを問題視:

| カテゴリ | 性質 | 寿命 | 例 |
|---|---|---|---|
| (a) 決定記録 | 過去形 | 長 | 色割り当て純関数化 |
| (b) 探索 | 現在進行形 | 短（陳腐化） | 用語再考 / スマホ UI |
| (c) メタ決定 | 過去形 | 長 | 残存する設計臭 defer |

(a)(c) は安定、(b) は不安定。混ぜると:
1. **役割契約の弱化** — DESIGN.md の「なぜそうしたか」が「まだ決めてない」で薄まる
2. **volatility mismatch** — 安定コンテンツが陳腐化リスクに晒される
3. **query パターンの衝突** — 「決定は？」と「選択肢は？」という異なる reader 動機が grep で混ざる

### 棄却した代替案

| 案 | 却下理由 |
|---|---|
| **3 ファイル (DECISIONS + EXPLORING + DEFERRED)** | defer は決定の一種（条件付き）、分ける実益なし。overengineering |
| **タグ付け (`[DECIDED]` `[EXPLORING]`)** | タグ規律が折れやすい、既存無タグ retrofit コスト、ファイル分離のほうが物理的に強制力がある |
| **サブディレクトリ (`design/{decisions,exploring}.md`)** | 階層コスト、発見性低下、小リポで過剰 |
| **何もしない (現状維持)** | DESIGN.md が探索 dumping ground になる傾向が既に存在（LorentzArena 2+1 の用語再考セクションが前例）、将来さらに劣化する |

### Defer と決定を同居させる理由

defer は「X をやらないと決めた」という **decision** で、un-defer トリガーが明示されていれば安定性も (a) と同等。reader query 「X はなぜ放置？」と「X はどう決まった？」は同じ DESIGN.md を見れば両方答えられる。分けると却って参照コストが上がる。

### 適用方針

- **retroactive migration はしない**: 既存リポの既存 DESIGN.md は触らない
- **新規探索が発生したリポから順に導入**: LorentzArena 2+1 が初回適用（スマホ UI）
- **小リポ・決定しか書かないリポでは不要**: EXPLORING.md は任意ファイル

### 初回適用

- `LorentzArena/2+1/EXPLORING.md` 新規作成（2026-04-06）
- 内容: スマホ UI の設計思考（option space 分析、α/β 候補、open questions、un-shelve トリガー）
- 2+1/SESSION.md の「次にやること」からポインタ
