...
**setup.sh step 7 への影響:** 既存設計のまま機能する。新規端末では `odakin-prefs` clone 後に `~/Claude/CLAUDE.md` (target なし or regular file) を symlink に置換。冪等性の実地検証は次回 setup.sh 実行時に行う (TODO)。

**残痕:** `~/Claude/CLAUDE.md.pre-symlink-backup` を一時退避として残してある。次回起動時に正常動作を確認し次第削除する。
