...
## ~/Claude/CLAUDE.md の symlink 化 (未着手・要次セッション判断)

**現状:** `~/Claude/CLAUDE.md` は regular file (約 1900 bytes)。同時刻に odakin-prefs/CLAUDE.md (private リポ) が並立して存在している。setup.sh step 7 は odakin-prefs リポが clone 済みなら `~/Claude/CLAUDE.md` を odakin-prefs/CLAUDE.md への symlink に置換する設計だが、odakin-prefs リポが作成された 2026-04-06 より前に最後の setup.sh が走ったため、現在まで symlink 化されていない。

**統合すべき内容** (`~/Claude/CLAUDE.md` にあって odakin-prefs/CLAUDE.md にないもの):

1. 作業ディレクトリ宣言 (`~/Claude` をベースに使う旨)
2. プロジェクト構成ルール (タスクごとサブフォルダ・ルート直下にコード禁止・命名規約)
3. preview_start のリンク出力ルール (`http://localhost:<port>` を表示)
4. ユーザー情報 (実名・所属・メールアドレス・lab アカウント運用ルール) — **private 情報**

**統合戦略の選択肢:**

- **(a) 全マージ + symlink 置換:** 上記 1-4 を odakin-prefs/CLAUDE.md にマージし、`~/Claude/CLAUDE.md` を symlink に置換。最も筋が通るが、odakin-prefs/CLAUDE.md の「1 ルール = 1 ファイル」原則に反する (CLAUDE.md 本体が肥大化)
- **(b) 個別ファイル化 + symlink 置換:** 1-4 をそれぞれ odakin-prefs 配下の独立ファイル (`project-structure.md`, `preview-rules.md`, `user-profile.md` 等) に分割し、odakin-prefs/CLAUDE.md のテーブルに登録。`~/Claude/CLAUDE.md` を symlink 化。原則に整合するが管理ファイル数が増える
- **(c) 一部だけ移管:** ユーザー情報など private 性が強いものだけ odakin-prefs に出す。残りは `~/Claude/CLAUDE.md` に残す → この場合 symlink にはできず、現在の並立状態が固定化する

**判断のトリガー:** 次セッションで明示的に取り上げる。setup.sh step 7 の冪等性 (既存 regular file を symlink に置換する挙動) を事前に確認すること。

**注意点:** 統合中は両ファイルを手で並行管理する必要がある。git の rename 検出は効かないので慎重に。

---

## claude-config git history scrubbing (未着手・要次セッション判断)
...
**暫定方針案:** **見送り**。HEAD クリーン化で実用的な対応は完了したと見なす。
...
## CONVENTIONS.md / conventions/ 内の自己言及的 odakin 記述
