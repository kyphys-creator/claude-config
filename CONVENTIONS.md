...
5. 破壊的操作は事前にユーザー確認。自分のリポのみ操作
   - **macOS システムコマンドの禁止**: `tccutil reset`（全アプリの TCC 権限一括削除）、`defaults delete com.apple.*`、`csrutil disable` 等は settings.json の deny ルールでブロック済み。プライバシー・セキュリティ設定の変更はユーザーが手動で行う
