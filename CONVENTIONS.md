...
SESSION.md: タスク完了時、重要な判断時、ファイル作成/大幅変更時、エラー発生時に更新。**認識の転換点**（方針変更、前提の修正、「変わったこと/変わっていないこと」の整理）では **作業の前に** SESSION.md に書く。出力テキストは揮発する。**棚卸し（目安80行以内）:** 完了 `[x]` を除去、実装詳細は git log に委任、重複を排除、恒久的決定は CLAUDE.md に移動。
