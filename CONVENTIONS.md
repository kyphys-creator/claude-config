...
7. **git-crypt 運用ルール**:
   - 鍵: `~/.gmail-mcp/git-crypt.key`（git 管理外）。REDACTED_REPO_LIST
   - 鍵のバックアップ: `REDACTED_BACKUP_PATH`（REDACTED_ENCRYPTION_INFO）
   - 新規暗号化リポ作成時: 同じ鍵を使い、CLAUDE.md に unlock 手順・鍵共有先・バックアップ復元手順を記載
   - git filter の絶対パス（`/usr/local/Cellar/git-crypt/0.8.0/bin/git-crypt`）は端末間で異なりうるため、新端末では `.git/config` の filter 設定を確認
