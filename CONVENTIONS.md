...
9. **Scheduled task の SKILL.md はリポを正本とし、`~/.claude/scheduled-tasks/` からは symlink で参照する。** コピーの二重管理は禁止（同期漏れの原因）。新規タスク登録時: `ln -s <base>/<repo>/skill/SKILL.md ~/.claude/scheduled-tasks/<taskId>/SKILL.md`
...
Google Calendar MCP はクラウドコネクタ（claude.ai 経由）。Gmail MCP と異なりローカル認証管理は不要。**注意: claude.ai ウェブチャットでのみ利用可能。Claude Code CLI からは MCP ツールとして利用不可（2026-03-30 確認）。Claude Code からカレンダー操作が必要な場合の設計は要検討。**

- **操作前の確認**: カレンダー一覧を取得し、対象カレンダーが正しいことを確認してからイベント操作する（§13「MCP ツール」の原則と同じ）
- **共有カレンダー命名**: `{共同研究者名}{odakin名}共同研究` 形式（例: 「石田尾田共同研究」）
- **共同研究者アカウント**: `gmail-mcp-config/collaborators.yaml` が正本。各プロジェクト CLAUDE.md には名前のみ記載し、メールアドレスは collaborators.yaml を参照
