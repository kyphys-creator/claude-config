...
| `DESIGN.md` | 設計判断とその理由（Why / 代替案 / トレードオフ）。判断が生じたら即記録 |
...
| 研究中に見つけた参照文献 | `physics-research/refs/refs.yaml`（`reftools.py` で管理） |
...
- **作業単位ごとに push する。** 1つの論理的なまとまり（1件の処理完了、1つの構造変更など）が終わったら次に移る前に commit + push。まとめ push はしない。
- **セッション開始時に push 疎通を確認する。** `git push --dry-run` で push 可能な状態を確かめてから作業開始。
- **push 障害は即座に解決する。** rebase コンフリクト・認証エラー等を放置しない。
...
- `preview_start` でサーバーを起動したら、`http://localhost:<port>` のリンクを出力すること（テスト用に別ブラウザで開けるように）
