...
| Section | Summary |
|---------|---------|
| §1 Repo creation & sync | `gh repo create` + initial commit recipe |
...
| §4 Git conventions | Branch strategy, commit messages, push protocol |
| §5 Safety rules | Guardrails for destructive operations, credentials |
| §6 Exhaustive verification | Mechanical checks before claiming completeness |
...
- §2: Adjust the required files to your project structure
- §4: Modify Git conventions for your workflow
- §5: Add or remove safety rules for your use case
...
| セクション | 内容 |
|-----------|------|
| §1 リポジトリ作成・同期 | `gh repo create` + 初期コミット手順 |
| §2 必須ファイル | `CLAUDE.md`（永続的指示書）、`SESSION.md`（揮発的作業ログ）、`.gitignore` |
| §3 自動更新プロトコル | SESSION.md / CLAUDE.md の自動更新タイミングとルール |
| §4 Git 規約 | ブランチ戦略、コミットメッセージ、push プロトコル |
| §5 安全規則 | 破壊的操作・機密情報に関するガードレール |
| §6 網羅性の検証 | 「全部」を主張する前の機械的検証 |
...
- §2: 必須ファイルをプロジェクト構造に合わせて調整
- §4: Git 規約をワークフローに合わせて変更
- §5: ユースケースに応じて安全規則を追加・削除
