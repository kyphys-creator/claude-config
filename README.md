...
See [CONVENTIONS.md](CONVENTIONS.md) for full details. Domain-specific rules are in [conventions/latex.md](conventions/latex.md) and [conventions/mcp.md](conventions/mcp.md).
...
See [§5 in CONVENTIONS.md](CONVENTIONS.md#5-安全規則絶対厳守) and [conventions/latex.md](conventions/latex.md) for LaTeX-specific rules.
...
Fork or clone this repo and edit [CONVENTIONS.md](CONVENTIONS.md) to match your workflow. The conventions are written in Japanese, but the structure is language-agnostic — translate or adapt as needed. `setup.sh` auto-detects your GitHub user — works as-is.
...
詳細は [CONVENTIONS.md](CONVENTIONS.md) を参照。ドメイン固有規約は [conventions/latex.md](conventions/latex.md)、[conventions/mcp.md](conventions/mcp.md)。
...
[CONVENTIONS.md §5](CONVENTIONS.md#5-安全規則絶対厳守) および [conventions/latex.md](conventions/latex.md)（LaTeX 固有ルール）を参照。
...
フォークまたは clone して、[CONVENTIONS.md](CONVENTIONS.md) を自分のワークフローに合わせて編集する。`setup.sh` は認証ユーザーを自動検出するのでそのまま動作する。
